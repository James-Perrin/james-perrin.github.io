...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var hear = readData("hear");
        var single = readData("single");
        var frequency = readData("frequency");
        var links = readData("links");
        var feel = readData("feel");
        var additional = readData("additional");

        res.render('showResults', {results: [hear, single, frequency, links, feel, additional]});

        console.log([hear, single, frequency, links, feel, additional]);
    });
    ...
    app.post('/niceSurvey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key === "links") && (json[key].length >= 2)){
